...
class MailingList(base.Base):
    ...
    def test_target_mailing_list(self):
        ...
        for commit in MailingList.commits:
            ...
            if match:
                self.fail(
                    "Series sent to the wrong mailing list",
                    "Check the project's README (%s) and send the patch to the indicated list"
                    % match.group("project"),
                    commit,
                )
            ...
            for project in [self.bitbake, self.doc, self.oe, self.poky]:
                if base_path in project.paths:
                    self.fail(
                        ...
                        "Send the series again to the correct mailing list (ML)",
                        data=[
                            (
                                "Suggested ML",
