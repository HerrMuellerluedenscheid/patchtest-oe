...
class LicFilesChkSum(base.Metadata):
    ...
    def test_lic_files_chksum_modified_not_mentioned(self):
        ...
        for pn in self.modified:
            pretest = patchtestdata.PatchTestDataStore[
                "pre%s-%s-%s" % (self.shortid(), self.metadata, pn)
            ]
            test = patchtestdata.PatchTestDataStore[
                "%s-%s-%s" % (self.shortid(), self.metadata, pn)
            ]
            ...
            if pretest != test:
                ...
                for commit in self.commits:
                    ...
                    self.fail(
                        'LIC_FILES_CHKSUM changed on target %s but there is no "%s" tag in commit message'
                        % (pn, self.lictag),
                        'Include "%s: <description>" into the commit message with a brief description'
                        % self.lictag,
