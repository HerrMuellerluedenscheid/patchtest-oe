...
class PatchUpstreamStatus(base.Base):
    ...
    def setUp(self):
        ...
        self.valid_status = ", ".join(
            parse_upstream_status.upstream_status_nonliteral_valid_status
        )
    ...
    def test_upstream_status_presence_format(self):
        ...
        for newpatch in PatchUpstreamStatus.newpatches:
            ...
            if not self.upstream_status_regex.search(payload):
                self.fail(
                    "Added patch file is missing Upstream-Status in the header",
                    "Add Upstream-Status: <Valid status> to the header of %s"
                    % newpatch.path,
                    data=[
            ...
            for line in payload.splitlines():
                ...
                if self.upstream_status_regex.search(line):
                    if parse_upstream_status.inappropriate_status_mark.searchString(
                        line
                    ):
                        try:
                            parse_upstream_status.upstream_status_inappropriate_info.parseString(
                                line.lstrip("+")
                            )
                        except pyparsing.ParseException as pe:
                            self.fail(
                                "Upstream-Status is Inappropriate, but no reason was provided",
                                "Include a brief reason why %s is inappropriate"
                        ...
                        try:
                            parse_upstream_status.upstream_status_submitted_info.parseString(
                                line.lstrip("+")
                            )
                        except pyparsing.ParseException as pe:
                            self.fail(
                                "Upstream-Status is Submitted, but it is not mentioned where",
                                "Include where %s was submitted"
                                % os.path.basename(newpatch.path),
                                data=[
                                    ("Current", pe.pstr),
                                    (
                        ...
                        try:
                            parse_upstream_status.upstream_status.parseString(
                                line.lstrip("+")
                            )
                        except pyparsing.ParseException as pe:
                            self.fail(
                                "Upstream-Status is in incorrect format",
                                "Fix Upstream-Status format in %s"
                                % os.path.basename(newpatch.path),
                                data=[
                                    ("Current", pe.pstr),
                                    ("Standard format", self.standard_format),
